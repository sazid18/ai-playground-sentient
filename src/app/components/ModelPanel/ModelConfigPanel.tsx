...
export function ModelConfigPanel() {
  const {modelConfig, updateModelConfig, inputConfigs, exportModelConfig, importModelConfig} = useModelConfig();
  ...
  const handleInputChange = (id: string, newValue: string, min: number, max: number, decimalPlaces: number) => {
    if (newValue === '') {
      updateValue(id, min)
      return
    }
  ...
  const handleExport = () => {
    try {
      ...
      a.href = url
      a.download = `ModelConfig-${new Date().toLocaleTimeString().replaceAll(':','-')}.json`
  ...
  return (
    <Card className="w-full max-w-lg">
          ...
          <div key={input.id} className="grid gap-2">
            ...
            <div className="flex items-center gap-4">
              <Input
                ...
                id={input.id}
                inputMode="decimal"
                value={formatValue(values[input.id] || input.min, input.decimalPlaces)}
                onChange={(e) => handleInputChange(input.id, e.target.value, input.min, input.max, input.decimalPlaces)}
                className="w-24"
              />
              <Slider
                min={input.min}
                ...
                onValueChange={(newValue) => handleSliderChange(input.id, newValue)}
                className="flex-1"
              />
            </div>
          </div>
      ...
      <div>
          <Button onClick={handleExport} className="mr-2">Export Settings</Button>
        ...
        <Button onClick={handleSave} disabled={!isChanged}>Save Changes</Button>
      </CardFooter>
    </Card>
  )
}
